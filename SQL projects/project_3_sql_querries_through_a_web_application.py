...
def findAirportVisitors(x, a, b):
    ...
    query = """
        select distinct rpt.name, count(distinct p.id)
        from
	        flights fl,
            passengers p,
            flights_has_passengers fhp,
...
def insertNewRoute(x, y):
    ...
    #print(rlnID[0][0], sourceID[0][0], destRpt[0][0])
    inTuple = ("1329", rlnID[0][0], sourceID[0][0], destRpt[0][0])
