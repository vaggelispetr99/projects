...
app.get("/jokes/:id", (req, res) => {
  ...
  // res.send(JSON.stringify(selectedJoke));
});
...
app.get("/filter", (req, res) => {
  ...
  if (type != undefined){
    ...
    // res.send(JSON.stringify(filteredJokes));
  } else {
    res.json(jokes);
...
app.post("/jokes", (req, res) => {
  ...
  var newId = jokes[jokes.length - 1].id + 1;
  var newJoke = {
    id: newId,
    jokeText: newText,
    jokeType: newType,
...
app.put("/jokes/:id", (req, res) => {
  ...
  var updateJoke = jokes.find((joke) => joke.id === id);
  if (updateJoke != undefined){
    updateJoke.jokeText = updateText;
    updateJoke.jokeType = updateType;
    res.json(jokes[id - 1]);
    // res.send(JSON.stringify(jokes[id - 1]));
  } else {
    var errortext = {
...
app.patch("/jokes/:id", (req, res) => {
  ...
  var updateJoke = jokes.find((joke) => joke.id === id);
  if (updateJoke != undefined){
    updateJoke.jokeText = updateText || updateJoke.jokeText;
    updateJoke.jokeType = updateType || updateJoke.jokeType;
    res.json(jokes[id - 1]);
  } else {
    var errortext = {
      error: "Not existing ID",
...
app.delete("/jokes/:id", (req, res) => {
  var id = parseInt(req.params.id);
  var deleteIndex = jokes.findIndex((joke) => joke.id === id);
  if (deleteIndex > -1) {
    jokes.splice(deleteIndex, 1);
    res.status(200).send("OK");
  } else {
    res.status(404).json({error: `Joke with the ${id} not found. No Joke was deleted!`});
  }
});
...
app.delete("/all", (req, res) => {
  ...
  if (apikey === masterKey) {
    ...
    res.status(404).json({error: `Error with the authentication of the API key. Jokes were not deleted!`});
  }
