...
/*
    MAX_RECORDS_IN_BLOCK is the maximun number of records, a block
    can hold, given that it also contains the HP_block_info
    struct, at the end of it.
    METADATA_POSITION, is where the last HP_block_info
    struct, should be indexed, if the block contained only
    HP_block_info structs.
*/
#define MAX_RECORDS_IN_BLOCK ((int)((BF_BLOCK_SIZE - sizeof(HP_block_info))/sizeof(Record)))
#define METADATA_POSITION ((int)((BF_BLOCK_SIZE/sizeof(HP_block_info))-1))

/* Η δομή HP_info κρατάει μεταδεδομένα που σχετίζονται με το αρχείο σωρού*/
typedef struct {

    int file_descriptor, last_block_id;
    BF_Block *info_block;
} HP_info;
...
int HP_GetAllEntries(
    HP_info* header_info, /* επικεφαλίδα του αρχείου*/
    int value /* η τιμή id της εγγραφής στην οποία πραγματοποιείται η αναζήτηση*/);
