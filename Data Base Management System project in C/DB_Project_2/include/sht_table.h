...
#define MAX_RECORDS_IN_SHT_BLOCK ((int)((BF_BLOCK_SIZE - sizeof(SHT_block_info))/sizeof(SHT_Record)))
#define SHT_METADATA_POSITION ((int)((BF_BLOCK_SIZE/sizeof(SHT_block_info))-1))
...
typedef struct {
    int file_descriptor, main_file_descriptor, last_block_id, buckets_num;
    int *hash_table;
    BF_Block *info_block;
} SHT_info;

typedef struct {
    int records_in_block, block_id;
    int next_block_id;
} SHT_block_info;
